"""
FUTURISTICS - Smart India Hackathon (SIH 2026) 9-Slide Pure DSP Presentation Generator
...
- Slide 1: Official Title Page (Times New Roman, 18pt content, Transparent Logos, ID 26147, Space Technology)
- Slide 2: Proposed Solution & 4-Pillar Pure DSP Architecture (Traditional Manual EW vs SPECTRA Pure DSP)
- Slide 3: Technical Approach & 4-Zone Pure DSP System Architecture (High-Res Architecture Diagram & Proofs)
- Slide 4: Feasibility, Risk Analysis & Pure DSP Mitigation Matrix (Technical/Operational Feasibility & Mitigation)
- Slide 5: Quantifiable ROI, Ecosystem Benefits & Space Agency Impact (4 Stat Cards & Target Beneficiaries)
- Slide 6: Modular DSP Signal Processing Pipeline & Architecture (3 Large End-to-End Workflow Cards)
- Slide 7: Research Standards, Literature Citations & Space Validation (Academic Citations & Real Benchmarks)
- Slide 8: Live Prototype Showcase & Production Tech Stack (Real UI Screenshots & Production Pillars)
- Slide 9: Detailed Technology Stack & Team FUTURISTICS Credentials (Architecture & Domain Specialists)
"""
...
HEX_IMG = os.path.join(ASSETS_DIR, "crop_interleaver_hex_clean.png")
...
# =============================================================
# SLIDE 6: MODULAR DSP SIGNAL PROCESSING PIPELINE (18pt)
# =============================================================
def build_slide_6(prs):
    blank = prs.slide_layouts[6]
    slide = prs.slides.add_slide(blank)
    add_header(slide, "MODULAR DSP SIGNAL PROCESSING PIPELINE & ARCHITECTURE")

    # 3 Large Process Cards Side-by-Side (w=3.85 each, h=5.65)
    modules = [
    ...
    for title, sub_features, col, x in modules:
        card = create_card(slide, x, 1.35, 3.85, 5.65, COLOR_WHITE, col, 1.2)
        strip = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(x), Inches(1.35), Inches(3.85), Inches(0.42))
        set_shape_fill(strip, col)
        strip.line.fill.background()
        p = strip.text_frame.paragraphs[0]
        p.text = title
        ...
        tb = slide.shapes.add_textbox(Inches(x+0.12), Inches(1.85), Inches(3.61), Inches(5.05))
        tf = tb.text_frame
        tf.word_wrap = True
        for i, (sf_title, sf_desc) in enumerate(sub_features):
            p = tf.paragraphs[0] if i == 0 else tf.add_paragraph()
            p.space_after = Pt(6)
            run1 = p.add_run()
            run1.text = f"✓ {sf_title}"
            run1.font.name = FONT_BODY
            run1.font.size = Pt(18)   # Strict 18pt Content
            run1.font.bold = True
            run1.font.color.rgb = COLOR_NAVY_DARK
...
# =============================================================
# SLIDE 7: RESEARCH STANDARDS, CITATIONS & SPACE VALIDATION (18pt)
# =============================================================
...
# =============================================================
# SLIDE 8: LIVE PROTOTYPE SHOWCASE & PURE DSP TECH STACK (18pt)
# =============================================================
def build_slide_8(prs):
    ...
    # Right: Module 2 Screenshot (w=5.9, h=3.6)
...
# =============================================================
# SLIDE 9: DETAILED TECH STACK & TEAM FUTURISTICS (18pt)
# =============================================================
...
def main():
    print("=" * 60)
    print("Generating 9-Slide Pure DSP Presentation for SPECTRA (Zero AI)...")
    print("Content Font Size: Strict 18pt across all slides with adjusted layout")
    print("=" * 60)
    ...
    print("Building Slide 6: Modular DSP Signal Processing Pipeline (18pt)...")
    build_slide_6(prs)

    print("Building Slide 7: Research Standards & Space Defense Validation (18pt)...")
    build_slide_7(prs)

    print("Building Slide 8: Live Prototype Showcase & Production Tech Stack (18pt)...")
    build_slide_8(prs)

    print("Building Slide 9: Detailed Tech Stack & Team FUTURISTICS (18pt)...")
    build_slide_9(prs)

    # Primary file requested by user
    primary_file = "SPECTRA_SIH_2026_Presentation.pptx"
    prs.save(primary_file)
    print("=" * 60)
    print(f"Presentation saved successfully to: {os.path.abspath(primary_file)}")
    print(f"File size: {os.path.getsize(primary_file):,} bytes")

    # Also save as explicit 9-slide pure DSP named file
    puredsp_file = "SPECTRA_SIH_2026_PureDSP_9Slide_Presentation.pptx"
    prs.save(puredsp_file)
    print(f"Separate Pure DSP file saved to: {os.path.abspath(puredsp_file)}")

    # Frontend copies
    frontend_primary = os.path.join("frontend", primary_file)
    frontend_puredsp = os.path.join("frontend", puredsp_file)
    try:
        prs.save(frontend_primary)
        prs.save(frontend_puredsp)
        print(f"Frontend copies updated at: {frontend_primary} and {frontend_puredsp}")
    except Exception as e:
        print(f"Note on frontend copies: {e}")
    print("=" * 60)
